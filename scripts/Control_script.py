...
def apagar_salidas(instrumento):
    instrumento.write('SOUR1:OUTP OFF;SOUR2:OUTP OFF')
    time.sleep(0.1)

def encender_salidas(instrumento):
    instrumento.write('SOUR1:OUTP ON;SOUR2:OUTP ON')
    time.sleep(.1)
...
setpoint = 25.0 
duracion_de_medicion = 60  # Segundos
...
while True:
    ...
    if lapso > i * periodo_de_muestreo:
        ...
        try:
            ...
            respuesta = ctemp.query('MEAS:TEMP?').strip()
            time.sleep(0.2)
        ...
        # 2. Leer Duty Cycle
        try:
            respuesta_duty = ctemp.query('PID:DUTY?').strip()
            time.sleep(0.2)
